...
var ExternalModuleNamePlugin = /** @class */ (function (_super) {
    ...
    ExternalModuleNamePlugin.prototype.onDeclaration = function (context, reflection, node) {
        if (reflection.kindOf(abstract_1.ReflectionKind.ExternalModule)) {
            var comment = getRawComment_1.getRawComment(node);
            // Look for @module
            var match = /@module\s+([\w\u4e00-\u9fa5\.\-_/@"]+)/.exec(comment);
            if (match) {
                // Look for @preferred
                var preferred = /@preferred/.exec(comment);
                reflection.comment = comment_1.parseComment(comment);
    ...
    ExternalModuleNamePlugin.prototype.onBeginResolve = function (context) {
        ...
        var refsArray = Object.keys(projRefs).reduce(function (m, k) {
            m.push(projRefs[k]);
            return m;
        }, []);
        // Process each rename
        this.moduleRenames.forEach(function (item) {
            ...
            if (item.preferred) {
                if (renaming.comment) {
                    var commentTags = renaming.comment.tags.filter(function (item) { return item.tagName == 'preferred'; });
